      ...
      >
        ...
        {DAYS.map((dayName, d) => (
          <g key={`day-${d}`}>
            <text
              x={0}
              y={topPadding + d * cellSize + cellSize / 2 + 3}
              fontSize={10}
              fill="var(--color-text-secondary)"
            >
              {dayName}
            ...
            {Array.from({ length: 24 }, (_, h) => {
              ...
              return (
                <circle
                  ...
                  onMouseEnter={() =>
                    count > 0 && setHovered({ day: d, hour: h, count, x: cx, y: cy })
                  }
                  onMouseLeave={() => setHovered(null)}
                />
              );
...
export function CommitVelocity({ data }: { data: WeekData[] }) {
  ...
  if (data.length === 0) {
    return (
      <p className="text-sm text-text-secondary text-center py-6">No commit data available.</p>
    );
  }
  ...
  for (const p of points) {
    ...
    if (m !== lastMonth) {
      monthLabels.push({ label: MONTHS[m], x: p.x });
      lastMonth = m;
    }
  }
  ...
  return (
    <div className="overflow-x-auto relative" ref={containerRef}>
      ...
      >
        ...
        {monthLabels.map((m, i) => (
          <text key={i} x={m.x} y={height - 4} fontSize={9} fill="var(--color-text-secondary)">
            {m.label}
          </text>
        ))}
      </svg>
...
export function CodeFrequency({ data }: { data: FileFreqData[] }) {
  ...
  if (data.length === 0) {
    return (
      <p className="text-sm text-text-secondary text-center py-6">No file change data available.</p>
    );
  }
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (m !== lastMonth) {
      monthLabels.push({ label: MONTHS[m], x: padding.left + i * gap });
      lastMonth = m;
    }
  }
      ...
      <div className="overflow-x-auto relative">
        {hd && (
          <Tooltip
            ...
            y={
              (baseline / height) *
                (((containerRef.current?.clientWidth || width) * height) / width) -
              20
            }
            containerRef={containerRef}
          />
...
export function LanguageBreakdown({ data }: { data: LanguageData[] }) {
  ...
  if (data.length === 0) {
    return (
      <p className="text-sm text-text-secondary text-center py-6">No language data available.</p>
    );
  }
  ...
  const items = [
    ...top,
    ...(otherCount > 0
      ? [{ language: "Other", count: otherCount, percentage: Math.round(otherPct * 10) / 10 }]
      : []),
  ];
      ...
      <div className="mt-3 space-y-1">
        {items.map((item, i) => (
          <div
            ...
            onMouseLeave={() => setHoveredIdx(null)}
          >
            <span className="text-xs text-text-secondary w-24 text-right truncate shrink-0">
...
interface ContributorTimelineData {
  email: string;
  name: string;
...
export function ContributorTimeline({ data }: { data: ContributorTimelineData[] }) {
  ...
  if (data.length === 0) {
    return (
      <p className="text-sm text-text-secondary text-center py-6">No contributor data available.</p>
    );
  }
  ...
  for (const w of weekList) {
    ...
    if (m !== lastMonth) {
      monthLabels.push({
        label: MONTHS[m],
        x: labelWidth + ((w - weekMin) / weekRange) * dotAreaWidth,
      });
      lastMonth = m;
    }
  }
  ...
  return (
    <div className="overflow-x-auto relative" ref={containerRef}>
      ...
      >
        {monthLabels.map((m, i) => (
          <text key={i} x={m.x} y={12} fontSize={9} fill="var(--color-text-secondary)">
            {m.label}
          </text>
        ))}
        {data.map((contributor, i) => {
          ...
          return (
            <g key={contributor.email}>
              ...
              >
                {contributor.name.length > 16
                  ? contributor.name.slice(0, 16) + "..."
                  : contributor.name}
              </text>
              {contributor.weeks.map((w) => {
                ...
                const isDotHovered =
                  hovered?.email === contributor.email && hovered?.week === w.week;
                return (
                  <circle
                    key={w.week}
                    cx={cx}
