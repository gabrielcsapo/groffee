...
export function ExploreList({ initialRepos }: { initialRepos: Repo[] }) {
  ...
  const fetchRepos = useCallback((q: string) => {
    if (!q) {
  ...
  return (
    <div className="max-w-4xl mx-auto mt-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-text-secondary">Discover public repositories on this Groffee instance.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="border border-border rounded-lg overflow-hidden bg-surface">
          {repos.map((repo, i) => (
            <div key={repo.id} className={`px-4 py-4 ${i < repos.length - 1 ? 'border-b border-border' : ''} hover:bg-surface-secondary transition-colors`}>
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <Link to={`/${repo.owner}/${repo.name}`} className="text-base font-semibold text-text-link hover:underline">
        ...
        <div className="border border-border rounded-lg p-12 text-center bg-surface">
          ...
          {initialQuery ? (
            ...
            <>
              <h3 className="text-sm font-medium text-text-primary mb-1">No public repositories yet</h3>
              <p className="text-xs text-text-secondary">
