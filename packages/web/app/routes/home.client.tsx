  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <Link to="/explore" className="text-sm text-text-link hover:underline">Explore all</Link>
      </div>
      <div className="border border-border rounded-lg overflow-hidden bg-surface">
        {repos.map((repo, i) => (
          <div key={repo.id} className={`px-4 py-3 ${i < repos.length - 1 ? 'border-b border-border' : ''} hover:bg-surface-secondary transition-colors`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <Link to={`/${repo.owner}/${repo.name}`} className="text-sm font-semibold text-text-link hover:underline">
                  {repo.owner}<span className="text-text-secondary font-normal">/</span>{repo.name}
                </Link>
                <span className="badge badge-public">Public</span>
              </div>
              {repo.updatedAt && (
                <span className="text-xs text-text-secondary">Updated {timeAgo(repo.updatedAt)}</span>
              )}
            </div>
            {repo.description && (
  ...
  return (
    <div className="text-center mb-12">
      ...
      <p className="text-text-secondary mb-6">
        Self-hosted git platform. Create repositories, collaborate with your team, and manage your code.
      </p>
    </div>
  )
}
