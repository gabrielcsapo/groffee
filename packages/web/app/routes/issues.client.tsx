...
export function IssuesList({ owner, repo, initialIssues }: { owner: string; repo: string; initialIssues: Issue[] }) {
  ...
  return (
    <div className="max-w-4xl mx-auto mt-4">
      ...
      {loading ? (
        ...
        <div className="border border-border rounded-lg overflow-hidden bg-surface">
          {issues.map((issue, i) => (
            <div key={issue.id} className={`px-4 py-3 ${i < issues.length - 1 ? 'border-b border-border' : ''} hover:bg-surface-secondary transition-colors`}>
              <div className="flex items-start gap-3">
                <svg className={`w-4 h-4 mt-0.5 flex-shrink-0 ${issue.status === 'open' ? 'text-success' : 'text-danger'}`} fill="currentColor" viewBox="0 0 16 16">
