...
export function PullDetailLayout({
  ...
  async function saveTitle() {
    ...
    if (res.ok) {
      setPr({ ...pr, title: editTitle.trim(), editCount: (pr.editCount || 0) + 1, lastEditedAt: new Date().toISOString() })
      setEditing(false)
    }
  }
  ...
  const statusBadge = pr.status === 'open' ? 'badge-open' : pr.status === 'merged' ? 'badge-merged' : 'badge-closed'
        ...
        <div className="mb-6">
          {editing ? (
            <div className="mb-4 flex items-center gap-2">
              ...
              />
              <button onClick={saveTitle} disabled={!editTitle.trim()} className="btn-primary btn-sm">Save</button>
              <button onClick={() => setEditing(false)} className="btn-sm rounded-md border border-border text-text-secondary hover:text-text-primary font-medium">Cancel</button>
            </div>
          ) : (
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="text-sm text-text-secondary">
              ...
              <code className="mx-1 px-1.5 py-0.5 bg-surface-secondary rounded text-xs">{pr.sourceBranch}</code>
              into
              <code className="mx-1 px-1.5 py-0.5 bg-surface-secondary rounded text-xs">{pr.targetBranch}</code>
            </span>
            {pr.editCount && pr.editCount > 0 && (
              <span className="text-xs text-text-secondary" title={`Edited ${pr.editCount} time${pr.editCount > 1 ? 's' : ''}${pr.lastEditedAt ? ` - last ${timeAgo(pr.lastEditedAt)}` : ''}`}>
                (edited)
              </span>
            )}
          </div>
