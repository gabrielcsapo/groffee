...
export default function NewPullRequest() {
  ...
  return (
    <div className="max-w-2xl mx-auto mt-4">
      {error && (
        <div className="mb-4 p-3 rounded-md bg-red-50 border border-danger/30 text-danger text-sm">{error}</div>
      )}
      ...
      <form onSubmit={handleSubmit} className="bg-surface border border-border rounded-lg p-6">
        ...
        <div className="flex gap-4 mb-4 pb-4 border-b border-border">
          ...
          <div className="flex-1">
            ...
            >
              <option value="">Select branch</option>
              {branches.filter(b => b !== targetBranch).map(b => <option key={b} value={b}>{b}</option>)}
            </select>
          </div>
        </div>
